#!/usr/bin/env python3
"""
Добавляет в колоду слова из data/extra_words3.json и переносит их сцены
...
def main():
    ...
    new = json.load(open(D("data", "extra_words3.json"), encoding="utf-8"))
    have = {norm(c["pt"]) for c in cards}
    nid = max(c["id"] for c in cards)

    # vez уже есть — у него появилось второе значение «очередь»
    for c in cards:
        if c["pt"] == "vez" and "очередь" not in c["ru"]:
            c["ru"] = "раз (случай); очередь"
            print("vez: перевод дополнен →", c["ru"])
    ...
    print(f"с исправленным переводом: {sum(1 for w in new if w.get('fix'))}")
