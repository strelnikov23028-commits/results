#!/usr/bin/env python3
"""
Добавляет в колоду слова из data/extra_words8.json.

Отличие от прежних добавлялок: дубли ищутся по точному написанию,
вместе с диакритикой. Прежняя проверка сравнивала слова без акутов,
из-за чего país (страна) отбрасывался как повтор pais (родители).
"""
import json, os

ROOT = os.path.dirname(os.path.abspath(__file__))
D = lambda *p: os.path.join(ROOT, *p)
SRC = "extra_words8.json"
...
def main():
    ...
    have = {c["pt"] for c in cards}          # точное сравнение, с диакритикой
